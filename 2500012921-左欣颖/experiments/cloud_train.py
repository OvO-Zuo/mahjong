"""Cloud training: SL->RL with best features (meld+wall, 11ch) on Ascend NPU."""
...
# ═══════════════════════════════════════════════
# Feature Agent (meld 10-13 + wall 14)
# ═══════════════════════════════════════════════
class MeldWallAgent(MahjongGBAgent):
    ...
    def _draw(self, tile):
        ...
        self.isAboutKong=False; self.hand.append(tile); self._upd()
        self.valid=[]
        ...
        self._fill_feat(); return self._obs()
    ...
    def _play(self, p, tile):
        ...
        self.valid.append(0); self._fill_feat(); return self._obs()
...
def main():
    ...
    if not os.path.exists(sl_path):
        print(f'WARNING: SL model not found at {sl_path}', flush=True)

    print(f'Cloud training: meld+wall (11ch), {N_EP} episodes', flush=True)
    ...
    if os.path.exists(sl_path):
        ...
        model.load_sl_tower(sd, 11)
        print('SL weights loaded', flush=True)
    model = model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)

    env = MahjongGBEnv(config={'agent_clz': MeldWallAgent})
    ...
    for ep in range(N_EP):
        ...
        for s in range(0,len(aa),bs):
            ...
            for _ in range(ppe):
                ...
                ratio=torch.exp(lp-olp); s1=ratio*db
        ...
        if (ep+1)%100 == 0 or ep == 0:
            e=time.time()-t0; hr=np.mean(hu_rates[-100:])*100; af=np.mean(max_fans[-100:])
            print(f'Ep{ep+1}/{N_EP} | loss={avg_loss:.4f} | hu={hr:.0f}% | fan={af:.0f} | {e:.0f}s', flush=True)

        if (ep+1) % CKPT_EVERY == 0:
            ckpt_path = os.path.join(WORK, f'ckpt_ep{ep+1}.pt')
            torch.save({'model': model.state_dict(), 'hu_rates': hu_rates, 'max_fans': max_fans, 'ep': ep+1}, ckpt_path)
            print(f'Checkpoint: {ckpt_path}', flush=True)

    # Save final
    result = {'hu_rates': hu_rates, 'max_fans': max_fans, 'elapsed': time.time()-t0,
              'final_hu_rate_100': float(np.mean(hu_rates[-100:])*100)}
    with open(os.path.join(WORK, 'cloud_result.json'), 'w') as f:
        json.dump(result, f)
    torch.save(model.state_dict(), os.path.join(WORK, 'final_model.pt'))
    print(f'Done. Final hu={result["final_hu_rate_100"]:.0f}%', flush=True)
